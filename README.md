...
- PII redaction layer (`backend/redact.py`) — names, emails, and usernames
  are tokenized before the dataset ever reaches the Claude API, and only
  rehydrated back to real values for the human viewing the trace panel.
  Claude never sees real PII, even mid-conversation. See
  [Honesty notes](#honesty-notes) for what this does and doesn't cover.
- Prompt caching on the system prompt and tool schemas — both are
  byte-identical across every turn of a request, so turns 2+ of the
  tool-use loop read them from cache instead of paying full input-token
  price each time. Helps most on multi-turn queries; a single-turn "yes/no"
  question sees little benefit since there's no repeat turn to hit cache.
...
- **The PII redaction layer covers known structured fields (email,
  username, displayName, assignee, reporterEmail) completely, plus
  emails/phone numbers and already-known names in free text.** It is not a
  general-purpose PII/NER scrubber — a name mentioned only in prose, never
  seen in a structured field, and not an email/phone pattern, will not be
  caught. See `backend/redact.py` for the exact scope.
